...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		flag = False
		self.stdout.write(f'Importing from "{options["file"].name}"...\n')
		count = 0
		for row in reader:
			# skip first line
			if not flag:
				...
				try:
					...
					self.stdout.write(self.style.NOTICE(f"Unable to import {row[0]}: {type(e)}: {e}"))
	...
	def process_row(self, row):
		...
		for tag in tagStrings:
			tagObj, _ = Tag.objects.get_or_create(text=tag)
